  ...
  return (
    <div style={{ width: "100%",alignItems: "center" }}>
      <FormControl sx={{ m: 1, minWidth: minWidth }}>
        <InputLabel color={otherProps.color} id="demo-simple-select-filled-label">
        ...
        >
          {options.map((item: any, index: number) => {
            return (
              <MenuItem key={index}  color={otherProps.color} value={item.name}>
                {checkBox ? (
                  <Checkbox  color={otherProps.color} checked={selectvalue.indexOf(item.name) > -1} />
                ) : null}
                <span
                  style={{
